import { world, system, Player, ItemStack, EquipmentSlot, EntityDamageCause, EntitySwingSource, EntityInitializationCause, } from "@minecraft/server";
...
function setSkillLevel(player, skillId, level) {
    player.setDynamicProperty(skillId, level);
}
/**
 * 특정 위치 주변의 나무 블록을 찾아 목록에 추가하는 헬퍼 함수
 */
function findNextLogs(loc, dimension, visited, targetList) {
 ...
 * 찹트리 메인 실행 함수
 */
export function breakConnectedLogs(startBlock, player) {
...
// 사과 드랍 실행 함수
function dropAppleWithChance(player, blockLoc, chancePercentage) {
...
function playMagicEffect(player) {
...
function showMasterTree(player, refundMode) {
    ...
    const buttonMap = [];
    // 모드 전환 버튼 등록
    form.button({ translate: refundMode ? "skill.ui.mode.forget" : "skill.ui.mode.learn" }, refundMode
    ...
    for (const key of disciplines) {
        ...
        buttonMap.push({ type: "header", key: key });
        const rootTxt = {
            rawtext: [
                { text: `${data.color}§l` },
                { translate: root.name },
        ...
        buttonMap.push({ type: "root", key: key });
        data.branches.forEach((b, i) => {
            ...
            buttonMap.push({ type: "branch", key: key, index: i });
        });
    }
    form.show(player).then((r) => {
        ...
        if (action.type === "toggle") {
            if (!refundMode)
                ...
                player.playSound("trial_spawner.open_shutter", { pitch: 1.5 });
            showMasterTree(player, !refundMode);
            return;
        }
        if (action.type === "header") {
            const categoryKey = action.key;
            ...
            detailForm.show(player).then((result) => {
                if (result.canceled)
                    return -1;
                else
                    reOpen();
        ...
        if (action.type === "root") {
            ...
            if (refundMode) {
                if (lvl > 0) {
                    const hasBranch = data.branches.some((b) => getSkillLevel(player, b.id) > 0);
                    if (hasBranch) {
...
function applyPassives(player) {
    ...
    if (getSkillLevel(player, "might_root") >= 1)
        totalHp += 1;
    if (totalHp >= 1)
...
world.afterEvents.entityHurt.subscribe((event) => {
    ...
    if (hurtEntity.hasTag("processing_extra_dmg"))
        return; // 추가피해 처리중이면 발생X
    if (attacker instanceof Player && projectile) {
        ...
        if (rangerLvl >= 1) {
            ...
            dim.spawnParticle("minecraft:critical_hit_emitter", hurtEntity.getHeadLocation());
            attacker.playSound("mob.zombie.woodbreak", { pitch: 1.4, volume: 0.3 });
            system.run(() => {
                if (hurtEntity.isValid)
                    hurtEntity.removeTag("processing_extra_dmg");
            });
        }
    }
        ...
        if (berserkLvl >= 3) {
            ...
            if (healAmount > 0) {
                ...
                attacker.dimension.spawnParticle("chunbae0523:blood", hurtEntity.getHeadLocation());
            }
        }
    }
});
    ...
    if (getSkillLevel(player, "ind_lumber") >= 1) {
        if (brokenBlockPermutation.type.id.includes("log")) {
            if (isPlayerPlaced(block)) {
                world.setDynamicProperty(`${block.location.x},${block.location.y},${block.location.z}`, undefined);
                return;
            }
            else {
                ...
                if (!toolId || !/(axe|AIOT|Paxel)/i.test(toolId))
                    return;
                // 찹트리 시작
...
world.afterEvents.playerSwingStart.subscribe((event) => {
    ...
    if ((swingSource === EntitySwingSource.Attack ||
        ...
        if (getSkillLevel(player, "agi_teleporter") >= 1 && player.isSneaking) {
            ...
            if (raycastResult) {
                ...
                system.runTimeout(() => {
                    if (!player.isValid)
                        return;
                    dim.spawnParticle("chunbae0523:teleportOut", player.getHeadLocation());
...
world.beforeEvents.playerInteractWithEntity.subscribe((event) => {
    ...
    if (getSkillLevel(player, "agi_overlord") >= 1 && player.isSneaking) {
        system.run(() => {
            ...
            world.structureManager.createFromWorld(structureName, target.dimension, target.location, target.location, {
                includeBlocks: false,
        ...
        if (itemComp && isSkillBook(itemComp.itemStack)) {
            ...
            if (getSkillLevel(player, "agi_dreamer") >= 1) {
                ...
                system.runTimeout(() => {
                    ...
                    targetDim.spawnParticle("chunbae0523:teleportIn", player.getHeadLocation());
                }, 1);
                entity.remove();
            }
        }
