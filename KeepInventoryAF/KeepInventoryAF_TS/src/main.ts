...
const inventoryDataBase: Record<string, SavedItem[]> = {};
...
world.afterEvents.entityDie.subscribe((event) => {
  ...
  if (deadEntity instanceof Player) {
    ...
    system.run(() => {
      ...
      droppedItems.forEach((itemEntity: Entity) => {
        const itemComponent = itemEntity.getComponent(
          "item",
...
world.afterEvents.playerSpawn.subscribe((event) => {
  ...
  player.sendMessage(
    "§l§c[※] §r§c사망 후 리스폰하지 않고 게임을 나갈 시 보유 중인 §4모든 아이템이 소멸§c됩니다.",
  );
